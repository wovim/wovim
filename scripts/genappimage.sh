...
VERSION=$("$ROOT_DIR"/build/bin/wovim --version | head -n 1 | grep -o 'v.*')
export VERSION
...
unset ARGV0
exec "$(dirname "$(readlink  -f "${0}")")/usr/bin/wovim" ${@+"$@"}
EOF
chmod 755 AppRun
...
export OUTPUT="${OUTPUT:-wovim-linux-$ARCH_OUTPUT.appimage}"

# If it's a release generate the zsync file
if [ -n "$TAG" ]; then
  export UPDATE_INFORMATION="gh-releases-zsync|wovim|wovim|$TAG|${OUTPUT}.zsync"
fi

# Generate AppImage.
#   - Expects: $ARCH, $APP, $VERSION env vars
#   - Expects: ./$APP.AppDir/ directory
#   - Produces: ./wovim-linux-$ARCH_OUTPUT.appimage
...
mv "$ROOT_DIR/build/${OUTPUT:-wovim-linux-$ARCH_OUTPUT.appimage}"* "$ROOT_DIR/build/bin"
